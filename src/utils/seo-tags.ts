// Vendored from @astrolib/seo (MIT). See seo-types.ts for why.
...
export const buildTags = (config: AstroSeoProps): string => {
  ...
  if (config.additionalLinkTags && config.additionalLinkTags.length > 0) {
    config.additionalLinkTags.forEach((linkTag) => {
      ...
      if (linkTag.media) attributes.media = linkTag.media;
      if (linkTag.type) attributes.type = linkTag.type;
      if (linkTag.color) attributes.color = linkTag.color;
      if (linkTag.as) attributes.as = linkTag.as;
      if (linkTag.crossOrigin) attributes.crossorigin = linkTag.crossOrigin;
